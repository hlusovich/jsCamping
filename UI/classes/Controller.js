...
class Controller {
  ...
  async showMessages(filterConfig = {}, skip = 0, top = 10, pagination = false) {
    ...
    if (this.checkedUserChat === 'Js Camping') {
      ...
      this.getMessagesTimeout = setInterval(async () => {
        const messages = await this.chatApiService
          .getPage(0, this.messageCount, filterConfig, true, localStorage.getItem('user'), this.checkedUserChat);
        this.messageList.innerHTML = '';
        this.messagesView
          .display(messages.map((item) => new Message(item, item.author)), localStorage.getItem('user'));
      }, 1500);
    }
  }
  ...
  clearFiltersFields() {
    ...
    this.showMessages({
      author: this.filterAuthor.value,
  ...
  toggleDisabledFormButton(button, name, password, passwordAgain = password) {
    if (name && this.validateName(name) && password && this.validatePassword(password)
            && passwordAgain && this.validatePassword(passwordAgain)
            && this.isPasswordsSame(password, passwordAgain)) {
  ...
  addFilterAreasListeners() {
    this.filterText.addEventListener('input', (e) => {
      if (this.changeFilterButtonsState(this.filterBtnSubmit, this.filterBtnCancel,
        e, this.filterDateFrom.value, this.filterDateTo.value, this.filterAuthor.value)) {
        this.clearFiltersFields();
      }
    });
    this.filterAuthor.addEventListener('input', (e) => {
      if (this.changeFilterButtonsState(this.filterBtnSubmit, this.filterBtnCancel,
        e, this.filterDateFrom.value, this.filterDateTo.value, this.filterText.value)) {
        this.clearFiltersFields();
      }
    });
    this.filterDateFrom.addEventListener('input', (e) => {
      if (this.changeFilterButtonsState(this.filterBtnSubmit, this.filterBtnCancel,
        e, this.filterDateTo.value, this.filterText.value, this.filterAuthor.value)) {
        this.clearFiltersFields();
      }
    });
    this.filterDateTo.addEventListener('input', (e) => {
      if (this.changeFilterButtonsState(this.filterBtnSubmit, this.filterBtnCancel,
        e, this.filterDateFrom.value, this.filterText.value, this.filterAuthor.value)) {
        this.clearFiltersFields();
      }
    });
  ...
  checkInFormListeners() {
    ...
    document.forms[1].name.addEventListener('input', () => {
      this.toggleDisabledFormButton(this.checkInFormButton, document.forms[1].name.value,
        document.forms[1].password.value, document.forms[1].passwordAgain.value);
    });
    ...
    document.forms[1].passwordAgain.addEventListener('change', () => {
      this.validatePassword(document.forms[1].passwordAgain.value,
        this.notificationCheckInPassword);
      this.isPasswordsSame(document.forms[1].password.value,
        document.forms[1].passwordAgain.value, this.notificationCheckInPasswordsIsSame);
    });
      ...
      if (this.addUser(document.forms[1].name.value.trim())) {
        try {
          ...
          await this.logIn(formData,
            document.forms[1].name.value.trim(), 'поздравляю,вы успешно зарегистрировались');
        } catch (e) {
  ...
  signInFormListeners() {
    document.forms[0].name.addEventListener('change', () => {
      ...
      this.toggleDisabledFormButton(this.signInFormButton, document.forms[0].name.value,
        document.forms[0].password.value);
    });
    ...
    document.forms[0].addEventListener('submit', async (e) => {
      ...
      if (this.chatApiService.getUser(document.forms[0].name.value.trim())) {
        try {
          ...
          this.notification.showNotification({ text: error.message, succesfull: false });
          this.makeSound('assets/sounds/уведомление.mp3');
        } finally {
