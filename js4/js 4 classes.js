...
class MessageList {
     ...
     * @returns {boolean} -true, if all fields have passed validation , in another cases returns false.
     */
    static validate(msg) {
        const validateObj = {
    ...
    getPage(skip = 0, top = 10, filterConfig = {}) {
        ...
        let visibleMessages = this._msgs.filter(item => {
     ...
     * @param {object} msg - object with  required field text,it can also contain fields isPersonal and to.
     *       {string} text - message text.
     *       {boolean} isPersonal  - if this field contains  true, then this object must contains field to.
     *       {string} to - name of the person to whom the message is being sent.
     * @returns {boolean}
     */
     ...
     * @returns {boolean} -  true ,if such a message exists and active user name equals to field name in this message.
     ...
     * @param {object} msg - with fields which we want to edit.You can change all fields except:author,id,createdAt.
     * @returns {boolean} -  true,if such a message exists and active user name equals to field name in this message and the new message is correct .
     * in other cases, return false
     */
    ...
    edit(id, msg) {
        ...
        let newMessage = changeMessageParams(oldMessage);
        if (MessageList.validate(newMessage)) {
            this._msgs = this._msgs.map(item => {
                    if (item.id === id) {
                        return newMessage;
                    }
                    return item;
...
console.log(messageList.getPage(0, 20));
console.log("изменяем текст сообщения с индексом 9 с валидными параметрами (текст про Игру престолов),оно  принадлежит нашему пользователю");
